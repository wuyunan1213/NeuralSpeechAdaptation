        ...
        if split_size == 1: # batch size smaller than row
            ...
            for e in range(epoch):
                ...
                for row in batch_list:
                    ...
                    model.fit(train_input[row], pred_label, epochs = epoch)
...
# def ss_expose(model, train, batch_size, epoch_size):
#     split_size = ceil((np.shape(train[0])[0])/batch_size)
#     if split_size == 1: # batch size smaller than row
#         pred_label = np.around(model.predict(train[0]),0).astype(int)
#         model.fit(train[0], pred_label, epochs = epoch_size, 
#                   batch_size = batch_size)
#     else:
#         train_input = np.array_split(train[0], split_size) 
#         for row in range(0,split_size):
#             pred_label = around(model.predict(train_input[row]),0).astype(int)
...
def plot_exp_results(fname, lrr = 12, c1 = 'blue', c2 = 'orange'):
    ...
    f2 = plot_mean_and_CI(rev_l_m, rev_l_se, mean2 = rev_h_m, se2 = rev_h_se, color_shading = c1,
                                    color_shading2=c2)
    f2name = train_fig_dir+'%s_Self_Reg_%s_.png'%(fname, lrr)
    f2.savefig(f2name)
