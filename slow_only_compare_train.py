...
def ff_nn_one(n_slow=40, n_inp = 30, lr_s = 1, activation = 'linear'):
    model = Sequential()
    ...
    out = Dense(1, activation="sigmoid", name="output")(s2)
    ...
    model.compile(optimizer = LRMultiplier(Adam(lr=5e-5), {'slow':lr_s, 's2': lr_s}),
                #SGD(lr=0.05),
                loss='binary_crossentropy',
                metrics=['accuracy'])    
    return model

def slow_fast_nn_one(lr_s = 1, n_inp = 30, n_slow=40, activation = 'linear', train_slow = False):
    inp = Input(shape = (n_inp,), name = 'input')
    slow = Dense(n_slow, activation=activation, name = 'slow')(inp)
    # fast = Dense(n_fast, kernel_regularizer=regularizers.l1(penalty), activation=activation, name = 'fast')(inp)
...
def set_fs_weights_one(slow_model, lr_s = 1, activation = 'linear', train_slow = False):
    fs_model = slow_fast_nn_one(lr_s = lr_s, activation = activation, train_slow = train_slow)
    ...
    fs_model.get_layer('s2').set_weights(sm_weights[2:4])
    # fs_model.get_layer('f2').set_weights(sm_weights[6:8])
    fs_model.get_layer('output').set_weights(sm_weights[4:6])
...
def plot_exp_results(fname, lrr = 12, c1 = 'blue', c2 = 'orange'):
    ...
    f2 = plot_mean_and_CI(rev_l_m, rev_l_se, mean2 = rev_h_m, se2 = rev_h_se, color_shading = c1,
                                    color_shading2=c2)
    f2name = train_fig_dir+'%s_slow_only_%s_.png'%(fname, lrr)
    f2.savefig(f2name)
