...
import config
import generate
import slow_only_compare_train as train_one
...
slow_model = train_one.ff_nn_one(lr_s = 1)
...
for i in range(n_exp):
    ...
    fs = train_one.set_fs_weights_one(slow_model, lr_s = lr_slow, train_slow = True)
    ...
    new = train_one.set_fs_weights_one(fs, lr_s = lr_slow, train_slow = True)
    ...
    new2 = train_one.set_fs_weights_one(new, lr_s = lr_slow, train_slow = True)
    ...
    pkl.dump(t1, open('can_test_slow.pkl', "ab"))
    pkl.dump(t2, open('rev_test1_slow.pkl', "ab"))
    pkl.dump(t3, open('rev_test2_slow.pkl', "ab"))



file_list = ['rev_test1_slow', 'can_test_slow', 'rev_test2_slow']
...
for i in range(n_exp):
    # ### Exposure phase training with canonical and reverse data
    self_fs = train_one.set_fs_weights_one(slow_model, lr_s = lr_slow, train_slow = True)
    r_l1, r_h1, rev_l1, rev_h1  = train_one.self_d2_test(self_fs, rev, low_d2_test, high_d2_test, 'Self_REVERSE1', 
                                    lr_s = lr_slow, batch_size = EXPOSURE_BATCH_SIZE, epoch= EXPOSURE_EPOCHS)
    print('REVERSE1 = ', r_l1, r_h1)
    
    self_new = train_one.set_fs_weights_one(self_fs, lr_s = lr_slow, train_slow = True)
    c_l, c_h, can_l, can_h = train_one.self_d2_test(self_new, canonical, low_d2_test, high_d2_test, 'Self_CANONICAL1', 
                                        lr_s = lr_slow, batch_size = EXPOSURE_BATCH_SIZE, epoch= EXPOSURE_EPOCHS)
    print('CANONICAL = ', c_l, c_h)
    self_new2 = train_one.set_fs_weights_one(self_new, lr_s = lr_slow, train_slow = True)
    ...
    pkl.dump(t1, open('can_test_self_slow.pkl', "ab"))
    pkl.dump(t2, open('rev_test1_self_slow.pkl', "ab"))
    pkl.dump(t3, open('rev_test2_self_slow.pkl', "ab"))



file_list = ['rev_test1_self_slow', 'can_test_self_slow', 'rev_test2_self_slow']
for file in file_list:
    train_one.plot_exp_results(file, lrr = lr_fast)
