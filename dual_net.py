...
def load_mnist(fullset=True):
    ...
    Returns:
        ...
        y: shape = 
    """
    mnist_dataset = scipy.io.loadmat()###load our simulated data
...
def init_dualnet(layers):
    ...
    for i in range(2, len(layers)+1):
        params[i-1] = {}
        if layers[i]['type'] == 'IP' and layers[i]['init_type'] == 'gaussian':
...
def dual_net(params, layers, data, labels):
    """
...
def relu_backward(output, input, layer):
    ...
    # implementation begins
    grad = np.zeros(input['data'].shape)
    grad[np.where(input['data'] > 0)] = 1
    input_od = np.multiply(grad,output['diff'])
    # implementation ends
...
def sgd_momentum(w_rate, b_rate, mu, decay, params, param_winc, param_grad):
    ...
    for i in range(len(params)):
        param_winc_[i+1]['b'] = mu*param_winc_[i+1]['b'] + b_rate*param_grad[i+1]['b']
        params_[i+1]['b'] = params_[i+1]['b']- param_winc_[i+1]['b']

        param_winc_[i+1]['w'] = mu*param_winc_[i+1]['w'] + w_rate*(param_grad[i+1]['w'] + decay*params_[i+1]['w'])
        params_[i+1]['w'] = params_[i+1]['w'] - param_winc_[i+1]['w']
    # implementation ends
