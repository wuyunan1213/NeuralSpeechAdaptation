...
class EpochLogger(Callback):
    ...
    A Logger that log  performance per batch.
    """
    def __init__(self, test_l, test_h, test_long, test_short):
        self.test_l = test_l
        self.test_h = test_h
        self.test_long = test_long
        self.test_short = test_short
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        self.pred_l.extend(self.model.predict(self.test_l)[0])
        self.pred_h.extend(self.model.predict(self.test_h)[0])
        self.pred_long.extend(self.model.predict(self.test_long)[0])
        self.pred_short.extend(self.model.predict(self.test_short)[0])
...
def paired_shuffle_split(x, split_size):
...
def self_d2_test(model, train, test_l, test_h, figType, test_long, test_short, lr_s = 1, lr_f = 10, batch_size = 40, epoch = 1):
    if (np.shape(train[0])[0]) % batch_size == 0:
        split_size = ceil((np.shape(train[0])[0])/batch_size)
        if split_size == 1: # batch size smaller than row
            raise "In this experiment, batch size shouldn't equal trainin sample size"
            ...
            acc = []
            loss = []
            history = EpochLogger(test_l, test_h, test_long, test_short)
            for e in range(epoch):
                np.random.shuffle(train)
                train_input = paired_shuffle_split(train, split_size)
                batch_list = list(range(0, split_size))
                np.random.shuffle(batch_list)
                for row in batch_list:
                    ...
                    pred_l.append(history.pred_l[0])
                    pred_h.append(history.pred_h[0])
                    pred_long.append(history.pred_long[0])
                    pred_short.append(history.pred_short[0])
                    # pred_l.append(model.predict(test_l)[0][0])
                    # pred_h.append(model.predict(test_h)[0][0])
    else:
        raise "Batch size not divisible by sample size"

    return pred_l, pred_h, pred_short, pred_long, acc, loss
